...
test.describe('API — catalog', () => {
  test(
    'TC-API-06 — the phone/notebook catalog contains the products UI tests depend on',
    { tag: ['@smoke'] },
    async ({ api }) => {
  ...
  test('TC-API-08 — an unknown category returns no items rather than erroring', { tag: ['@edge'] }, async ({
    api,
  }) => {
